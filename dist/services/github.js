...
export class GitHubService {
    ...
    getGitHubEnv() {
        ...
        if (!home) {
            ...
            // Last resort: use logname command or default
            if (!home) {
                home = process.env.USER ? `/Users/${process.env.USER}` : '/Users/jhchoi';
            }
        }
        return {
            ...process.env,
     ...
     */
    async createRepository(repoName, templateOwner, templateRepo = 'toybox') {
        try {
     ...
     */
    async enableGitHubPages(repoName, owner) {
        ...
        try {
            ...
            try {
                ...
                log.error('Failed to get existing GitHub Pages URL', {
                    ...
                    stdout: getUrlError?.stdout,
                    stderr: getUrlError?.stderr
                });
     ...
     */
    async getRepositoryInfo(repoName, owner) {
        try {
            const { stdout } = await this.execGh([
                'repo', 'view', repoName,
     ...
     */
    async triggerWorkflow(repoName, workflowFile = 'deploy.yml', owner) {
        try {
            // Get owner if not provided
            if (!owner) {
                owner = await this.getCurrentUser();
     ...
     */
    async createEmptyRepository(repoName, isPrivate = false) {
        try {
     ...
     */
    async getCloneUrl(repoName, useSSH = true) {
        try {
            const { stdout } = await this.execGh([
                'repo', 'view', repoName,
